...
Carisma: ${estadisticas.carisma}`)
    ...
    async Comandos(isMod, isAdmin, msg) {
        ...
        let command_boosters = [
            ...
            { name: '!meme', value: 'Podes generar un meme con un prompt o consulta. Ejemplo !meme <prompt>"' },
...
Que sea wallpaper para el celular o computadora.
    ...
    async SetupBirthdays(client, interaction) {
        ...
        try {
            ...
            if (bd.length > 0) {
                ...
                if (!chnl) {
                    const { channel } = await functions_on_discord.CreateChannelWithCategory(interaction, '🎂Feliz Cumpleaños🎂', '🎂Mensajes de cumpleaños🎂', 'Canal para recibir la felicitación de cumpleaños de los usuarios', 'Creación de la categoría de cumpleaños', 'Creación del canal de cumpleaños', [
                        {
                            id: guild.id,
    ...
    async BirthdayLib(client, interaction) {
    ...
    async CumpleañosLib(client, interaction) {
        ...
        if (usr.length == 0) {
            await interaction.reply({ content: `<@${userId}> no tiene registrado su cumpleaños.` });
        }
        else {
